...
import uk.gov.hmrc.specsteps.CheckYourAnswersStepDefSteps.{thenBackgroundSectionHasFollowing, verifyAboutTheTrustAnswers, verifyAboutYouAnswers}
...
class TrustCheckYourAnswerAboutTheTrustSpec extends BaseSpec {
  ...
  Feature("A trust is able to check their answers for About the Trust section before submitting page") {
    ...
    Scenario("About the Trust - CYA and send notification") {
      ...
      And("line 1 should have a label Your full name an answer with Trust full name and change URL ends with your-full-name/change")
       verifyAboutYouAnswers("1", "Your full name", "Trust full name", "your-full-name/change")
