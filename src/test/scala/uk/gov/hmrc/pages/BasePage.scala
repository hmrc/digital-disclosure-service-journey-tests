...
trait BasePage extends WebBrowser with Assertions with ScalaDsl with EN with ScalaFutures with StartUpTearDown {
  ...
  def findByclassName(elementclassName: String): WebElement =
    driver.findElement(By.className(getElement(elementclassName)))
  ...
  def waitForPresenceOfElement(element: By): WebElement =
  ...
  def verifyPageTitle(expectedTitle: String): Unit = {
    ...
    Assert.assertTrue(
      "Page Title not Verified. Expected: " + expectedTitle + "--- Actual: " + actualTitle,
  ...
  def verifyPageQuestion(expectedQHeader: String): Unit = {
    ...
    Assert.assertTrue(
      "Page Question not Verified. Expected:  " + expectedQHeader + "--- Actual:  " + actualQHeader,
      expectedQHeader == actualQHeader
    )
  }
  ...
  def authenticationByPass(): Unit = {
    ...
    //val nino = "AA000000B"
    findByID("authorityId").sendKeys("NIC12")
  ...
  def authenticationAgent(): Unit = {
    val reDirectUrl   = findByID("redirectionUrl")
    reDirectUrl.clear()
    reDirectUrl.sendKeys(Configuration.settings.baseUrl)
    val elements      = findByID("confidenceLevel").findElements(By.tagName("option"))
    elements.get(2).click()
    val affinityGroup = findByID("affinityGroupSelect").findElements(By.tagName("option"))
    affinityGroup.get(2).click()
    findByID("submit").click()
    if (driver.findElements(By.xpath("//*[@class='cbanner-govuk-cookie-banner']")).size() != 0) {
      driver.findElement(By.xpath("//button[contains(text(), 'Accept additional cookies')]")).click()
      driver.findElement(By.xpath("//button[contains(text(), 'Hide cookies message')]")).click()
  ...
  def navigateToHomePageForAgent(): Unit = {
    goTo(Configuration.settings.baseUrl)
    authenticationAgent()
  }
  ...
  def navigateToSpecificPageForAgent(specificPage: String): Unit = {
    navigateToHomePageForAgent()
    findByID("start").click()
    if (driver.getTitle.contains("Using this service")) {
      clickOnRadioButton("Make a notification first", "1")
      saveAndContinue()
  ...
  def verifyBulletText(expectedText: String, bulletNum: String): Unit = {
    ...
    Assert.assertTrue(
      "bullet Text is not Verified. Expected:  " + expectedText + "--- Actual:  " + actualText,
  ...
  def clickLink(element: String): Unit = {
  ...
  def verifyFocusOnTextbox(): Unit =
    try driver
      .findElement(By.xpath("//input[contains(@class,\"govuk-input\")]"))
  ...
  def verifyRadioButtonAndText(expectedText: String, bulletNum: String): Unit = {
    val elementInput =
      driver.findElements(By.xpath("//*[@class='govuk-radios__item']/input[@class='govuk-radios__input']"))
    val value        = bulletNum.toInt
    ...
    val elementLabel =
      driver.findElements(By.xpath("//*[@class='govuk-radios__item']/label[contains(@class,'govuk-radios__label')]"))
    val actualText   = elementLabel.get(value - 1).getText
    Assert.assertTrue(
      "Radio Text is not Verified. Expected: " + expectedText + "--- Actual:  " + actualText,
      expectedText == actualText
    )
  }
  ...
  def verifyPageHeading(expectedQHeader: String): Unit = {
    ...
    Assert.assertTrue(
      "Heading is not Verified. Expected:  " + expectedQHeader + "--- Actual:  " + actualQHeader,
      expectedQHeader.contains(actualQHeader)
    )
  }
  ...
  def verifyH2Header(expectedHeader: String): Unit = {
    ...
    Assert.assertTrue(
      "H2 Header is not Verified. Expected:  " + expectedHeader + "--- Actual:  " + actualHeader,
  ...
  def verifyH2HeaderNotDisplayed(expectedHeader: String): Unit = {
    ...
    Assert.assertTrue(
      "H2 Header is displayed. Expected:  " + expectedHeader + " should not be displayed --- Actual:  Header displayed",
      actualHeader.equals("")
    )
  }
  ...
  def clickOnRadioButton(expectedText: String, bulletNum: String): Unit = {
    ...
    Assert.assertTrue(
      "Radio Text is not Verified. Expected: " + expectedText + "--- Actual:  " + actualText,
      expectedText.trim == actualText.trim
    )
  ...
  def clickOnCheckBox(expectedText: String, bulletNum: String): Unit = {
    val elementLabel = driver.findElements(
      By.xpath("//*[@class='govuk-checkboxes__item']/label[contains(@class,'govuk-checkboxes__label')]")
    )
    val value        = bulletNum.toInt
    val actualText   = elementLabel.get(value - 1).getText
    Assert.assertTrue(
      "Checkbox Text is not Verified. Expected: " + expectedText + "--- Actual:  " + actualText,
      expectedText.trim == actualText.trim
    )

    val elementInput =
      driver.findElements(By.xpath("//*[@class='govuk-checkboxes__item']/input[@class='govuk-checkboxes__input']"))
    elementInput.get(value - 1).click()
  }
  ...
  def verifyFocusOnRadioButton(expectedText: String, bulletNum: String): Unit = {
    ...
    val elementInput =
      driver.findElements(By.xpath("//*[@class='govuk-radios__item']/input[@class='govuk-radios__input']"))
    Assert.assertTrue(
      "Focus is not on Radio Button",
      elementInput.get(value - 1).equals(driver.switchTo().activeElement())
    )
  }
  ...
  def verifyNewTabUrl(expectedQHeader: String): Unit = {
    ...
    driver.switchTo.window(newTab)
    Assert.assertTrue(
      "link is not opened in new tab",
      driver.getCurrentUrl.toLowerCase().contains(expectedQHeader.toLowerCase())
    )
    driver.close()
  ...
  def verifyErrorSummary(expectedError: String): Unit = {
    ...
    Assert.assertTrue(
      "Error Summary is not Verified.  Expected: " + expectedError + "--- Actual: " + actualError,
  ...
  def verifyBody(expectedText: String): Unit = {
    ...
    Assert.assertTrue(
      "Body Text is not Verified. Expected: " + expectedText + "--- Actual: " + actualText,
      expectedText == actualText
    )
  }
  def verifyErrorMessage(expectedError: String): Unit = {
    ...
    val actualError = question.getText
    Assert.assertTrue(
      "Error Message is not Verified. Expected: " + expectedError + "--- Actual: " + actualError,
      expectedError == actualError
    )
  }
  ...
  def verifyAddress(position: String, expectedText: String): Unit = {
    ...
    Assert.assertTrue(
      "Address is not Verified " + expectedText + " --- Actual:  " + actualText,
      expectedText == actualText
    )
  }
  ...
  def verifyHintText(expectedText: String): Unit = {
    ...
    Assert.assertTrue(
      "Hint text is not Verified" + expectedText + " --- Actual:  " + actualText,
      expectedText == actualText
    )
  }
  ...
  def verifySubmittedCaseRef(caseRef: String): Unit = {
    ...
    Assert.assertTrue(
      "CaseRef is not displayed, --- Expected: " + caseRef + " --- Actual: " + actualCaseRef,
      actualCaseRef.contains(caseRef)
    )
  }
  ...
  def verifyDisplayedCheckbox(expectedCount: String): Unit = {
    ...
    Assert.assertTrue(
      "CaseRef is not displayed, --- Expected: " + expectedCount + " --- Actual: " + actualCount,
      actualCount.toString.contains(expectedCount)
    )
  }
}
