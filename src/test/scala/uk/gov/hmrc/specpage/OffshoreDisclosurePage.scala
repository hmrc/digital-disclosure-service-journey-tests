...
trait OffshoreDisclosurePage extends BasePage {
  ...
  def enterCountry(textInput: String): Unit = {
    ...
    try {
      Driver.instance.findElement(By.id("country")).sendKeys(textInput)
    } catch
    {
  ...
  def selectFromCountryDropdown(stringtoSelect: String): Unit = {
    ...
    var actualtext=""
    try{
      val element = Driver.instance.findElements(By.xpath("//li[contains(@id,'country__option')]"))
      count = element.size()
  ...
  def checkTaskStatus(expectedStatus: String,fieldName: String): Unit = {
    ...
    val actualStatus = Driver.instance.findElement(By.xpath("(//span[contains(@class,\"hmrc-status-tag\")])["+count+"]")).getText
