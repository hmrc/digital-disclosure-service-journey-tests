...
trait CaseManagementPage extends BasePage {
  ...
  def clickEditCaseLink(): Unit = {
    Driver.instance.findElements(By.partialLinkText("Edit")).get(0).click()
  }

  def selectValueFromAffinityGroupDropdown(stringtoSelect: String): Unit = {
    val element = Driver.instance.findElements(By.xpath("//select[contains(@id,'affinityGroupSelect')]/option"))
  ...
  def selectValueFromConfidenceLevelDropdown(stringtoSelect: String): Unit = {
    val element = Driver.instance.findElements(By.xpath("//select[contains(@id,'confidenceLevel')]/option"))
    val count = element.size()
  ...
  def validateReference(position: String, expecteValue: String): Unit = {
    val element = Driver.instance.findElements(By.xpath("//tbody[@class='govuk-table__body']/tr/th"))
    val count = element.size()
  ...
  def validateType(position: String, expecteValue: String): Unit = {
    val element = Driver.instance.findElements(By.xpath("//tbody[@class='govuk-table__body']/tr/td[1]"))
    val count = element.size()
  ...
  def validateStatus(position: String, expecteValue: String): Unit = {
    val element = Driver.instance.findElements(By.xpath("//tbody[@class='govuk-table__body']/tr/td[3]"))
    val count = element.size()
