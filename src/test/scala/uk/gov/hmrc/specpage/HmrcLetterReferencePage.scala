...
trait HmrcLetterReferencePage extends BasePage {
  ...
  def isInputTextBoxDisplayed(): Unit = {
    Assert.assertTrue("Input Text Box is not displayed",Driver.instance.findElement(By.xpath("//input[contains(@class,\"govuk-input\")]")).isDisplayed.==(true))
  }
  def isBackButtonDisplayed(): Unit = {
    Assert.assertTrue("Back Button is not displayed", Driver.instance.findElement(By.className("govuk-back-link")).isDisplayed.==(true))
  }
