...
trait WhatIsTheCountryOfAddressPage extends BasePage {
  ...
  def verifyDropdownDisplayed(): Unit = {
    val element = Driver.instance.findElement(By.xpath("//*[@id='countryCode__listbox']"))
    Assert.assertTrue("Dropdown is displayed", element.isDisplayed)
  }

  def selectfromDropdown(stringtoSelect: String): Unit = {
    val element = Driver.instance.findElements(By.xpath("//li[contains(@id,'countryCode__option')]"))
  ...
  def verifyAddressHyperlink(expectedQHeader: String): Unit = {
    var actualHeader = ""
    val element = Driver.instance.findElements(By.xpath("//a[@class=\"govuk-link\"]"))
    element.forEach(e =>
