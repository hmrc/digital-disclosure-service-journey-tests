...
trait WhatIsYourEmailAddressPage extends BasePage {
  ...
  def verifyHintTextNotDisplayed(expectedQHeader: String): Unit = {
    var actualQHeader = ""
    try {
      val element = Driver.instance.findElement(By.xpath("//*[@class='govuk-hint']"))
  ...
  def enterEmailInTextBoxWithMaxLength(length: String): Unit = {
    ...
    Driver.instance.findElement(By.xpath("//input[contains(@class,\"govuk-input\")]")).sendKeys(textInput+"example.com")
  }
}